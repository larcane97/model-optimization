...
def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    ...
    # img_size = trial.suggest_categorical("img_size", [112,192,224,384,448])
    aug_type = trial.suggest_categorical("train_aug", ["simple_augment_train","randaugment_train"])
    level = trial.suggest_int("level", low=0, high=30, step=5)
    batch_size = trial.suggest_int("batch_size", low=16, high=32, step=16)
    lr = round(trial.suggest_float("lr",low=5e-5,high=1e-3,step=1e-5),6)
    return {
        "EPOCHS": epochs,
        "IMG_SIZE": 224,
...
def objective(trial: optuna.trial.Trial, device,model_cfg:str,model_name:str) -> Tuple[float, int, float]:
    """Optuna objective.
    Args:
        trial
    Returns:
    ...
    model_config = read_yaml(cfg=model_cfg)
    ...
    data_config["AUG_TRAIN"] = hyperparams['AUG_TYPE']
    ...
    mean_time = check_runtime(
        ...
        [model_config["input_channel"]] + [data_config["IMG_SIZE"],data_config["IMG_SIZE"]],
        device,
    )
    ...
    optimizer = optim.AdamW(parameters, eps=EPS, betas=BETAS,lr=BASE_LR, weight_decay=WEIGHT_DECAY)
    ...
    name_postfix = '+'.join([f"{k}:{v}" for k,v in hyperparams.items()])
    model_name = model_name + '_' + name_postfix
    log_dir = os.path.join("exp", model_name)
    ...
    loss, f1_score, acc_percent = trainer.test(model_instance, test_dataloader=val_loader)
...
def tune(gpu_id, model_name:str,model_cfg:str,storage: str = None):
    ...
    study = optuna.create_study(
        ...
        study_name="swinT_automl",
        sampler=sampler,
        storage=rdb_storage,
        load_if_exists=True,
    )
    study.optimize(lambda trial: objective(trial, device,model_cfg,model_name), n_trials=100)
    # study.optimize(lambda trial: objective(trial, device,model_cfg,model_name), n_trials=500)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_cfg", default="", type=str, help="model cfg")
    parser.add_argument("--gpu", default=0, type=int, help="GPU id to use")
    parser.add_argument("--storage", default="", type=str, help="Optuna database storage path.")
    args = parser.parse_args()
    tune(args.gpu, storage=args.storage if args.storage != "" else None,model_name=args.model_name, model_cfg = args.model_cfg)
