...
def search_model(trial: optuna.trial.Trial) -> List[Any]:
    ...
    if m1_stride == 2:
        n_stride += 1
    m1_activation = trial.suggest_categorical("m1/activation", ["ReLU", "Hardswish", "GiLU","SiLU"])
    if m1 == "Conv":
    ...
    if m2 == "Conv":
        ...
        m2_activation = trial.suggest_categorical(
            "m2/activation", ["ReLU", "Hardswish", "GiLU","SiLU"]
        )
    ...
    if m3 == "Conv":
        ...
        m3_activation = trial.suggest_categorical(
            "m3/activation", ["ReLU", "Hardswish", "GiLU","SiLU"]
        )
    ...
    if m4 == "Conv":
        ...
        m4_activation = trial.suggest_categorical(
            "m4/activation", ["ReLU", "Hardswish", "GiLU","SiLU"]
        )
    ...
    if m5 == "Conv":
        ...
        m5_activation = trial.suggest_categorical(
            "m5/activation", ["ReLU", "Hardswish", "GiLU","SiLU"]
        )
    ...
    if m6 == "Conv":
        ...
        m6_activation = trial.suggest_categorical(
            "m6/activation", ["ReLU", "Hardswish", "GiLU","SiLU"]
        )
    ...
    if m7 == "Conv":
        ...
        m7_activation = trial.suggest_categorical(
            "m7/activation", ["ReLU", "Hardswish", "GiLU","SiLU"]
        )
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    model_name = f"autoML_{trial.number}"
    log_dir = os.path.join("/opt/ml/code/exp", model_name)
...
def tune(gpu_id, storage: str = None):
    ...
    study_name = "autoML_model_optimization"
    
    if storage is not None:
        rdb_storage = optuna.storages.RDBStorage(url="sqlite:///{}.db".format(study_name))
    else:
        rdb_storage = None
